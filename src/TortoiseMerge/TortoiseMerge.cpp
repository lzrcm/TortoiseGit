...
BOOL CTortoiseMergeApp::InitInstance()
{
	...
	do
	{
		langDll.Format(L"%sLanguages\\TortoiseMerge%ld.dll", (LPCWSTR)CPathUtils::GetAppParentDirectory(), langId);
	...
	if (parser.HasKey(L"?") || parser.HasKey(L"help"))
	{
		...
		MessageBox(NULL, sHelpText, L"TortoiseGitMerge", MB_ICONINFORMATION);
		return FALSE;
	}
