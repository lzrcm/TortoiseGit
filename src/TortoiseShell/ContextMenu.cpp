...
void CShellExt::InsertGitMenu(BOOL istop, HMENU menu, UINT pos, UINT_PTR id, UINT stringid, UINT icon, UINT idCmdFirst, GitCommands com, UINT /*uFlags*/)
{
	...
	std::wstring verb = std::wstring(menutextbuffer);
	if (verb.find('&') != -1)
	{
                                              ...
                                              UINT uFlags)
{
	...
	bool bShowIcons = !!DWORD(CRegStdDWORD(L"Software\\TortoiseGit\\ShowContextMenuIcons", TRUE));
...
void CShellExt::AddPathCommand(tstring& gitCmd, LPCWSTR command, bool bFilesAllowed)
{
	gitCmd += command;
	gitCmd += L" /path:\"";
...
void CShellExt::AddPathFileDropCommand(tstring& gitCmd, LPCWSTR command)
{
	tstring tempfile = WriteFileListToTempFile();
	gitCmd += command;
	gitCmd += L" /pathfile:\"";
...
STDMETHODIMP CShellExt::InvokeCommand_Wrap(LPCMINVOKECOMMANDINFO lpcmi)
{
	...
	if (!files_.empty() || !folder_.empty())
	{
		...
		{
			std::wstring verb = std::wstring(MultibyteToWide(lpcmi->lpVerb));
			std::map<std::wstring, UINT_PTR>::const_iterator verb_it = myVerbsMap.lower_bound(verb);
	...
	case WM_MENUCHAR:
		{
			...
			{
				LPCWSTR resource = GetMenuTextFromResource((int)mySubMenuMap[It->first]);
				if (!resource)
					continue;
...
bool CShellExt::InsertIgnoreSubmenus(UINT &idCmd, UINT idCmdFirst, HMENU hMenu, HMENU subMenu, UINT &indexMenu, int &indexSubMenu, unsigned __int64 topmenu, bool bShowIcons, UINT /*uFlags*/)
{
	...
	auto I = files_.cbegin();
	if (wcsrchr(I->c_str(), '\\'))
		wcscpy_s(ignorepath, MAX_PATH, wcsrchr(I->c_str(), '\\')+1);
	else
	...
	if ((itemStates & ITEMIS_IGNORED) && (!ignoredprops.empty()))
	{
		...
		if (p!=-1)
		{
			...
			std::wstring verb = std::wstring(ignorepath);
		...
		if (wcsrchr(ignorepath, '.'))
		{
				...
				((ignoredprops.compare(maskbuf)==0) || (ignoredprops.find('\n', p)==p+wcslen(maskbuf)+1) || (ignoredprops.rfind('\n', p)==p-1)))
			{
				...
				std::wstring verb = std::wstring(maskbuf);
				myVerbsMap[verb] = idCmd - idCmdFirst;
				myVerbsMap[verb] = idCmd;
				myVerbsIDMap[idCmd - idCmdFirst] = verb;
				myVerbsIDMap[idCmd] = verb;
	...
	else if ((itemStates & ITEMIS_IGNORED) == 0)
	{
		...
		else
		{
			if (itemStates & ITEMIS_INGIT)
			{
				...
				swprintf_s(ignorepath, MAX_PATH, stringtablebuffer, files_.size());
				InsertMenu(ignoresubmenu, indexignoresub++, MF_BYPOSITION | MF_STRING , idCmd, ignorepath);
				std::wstring verb = std::wstring(ignorepath);
				myVerbsMap[verb] = idCmd - idCmdFirst;
				myVerbsMap[verb] = idCmd;
...
void CShellExt::RunCommand(const tstring& path, const tstring& command, LPCWSTR errorMessage)
{
