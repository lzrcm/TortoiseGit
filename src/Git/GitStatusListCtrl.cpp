...
public:
	CIShellFolderHook(LPSHELLFOLDER sf, const CTGitPathList& pathlist)
	{
		...
		for (int i = 0; i < pathlist.GetCount(); ++i)
			sortedpaths.insert((LPCWSTR)g_Git.CombinePath(pathlist[i].GetWinPath()));
	}
...
void CGitStatusListCtrl::AddEntry(CTGitPath * GitPath, WORD /*langID*/, int listIndex)
{
	...
	if(GitPath->m_Action & (CTGitPath::LOGACTIONS_REPLACED|CTGitPath::LOGACTIONS_COPY) && !GitPath->GetGitOldPathString().IsEmpty())
	{
		if (!abbreviateRenamings)
		{
			entryname += L' ';
			// relative path
			entryname.AppendFormat(from, (LPCWSTR)GitPath->GetGitOldPathString());
		}
		else
		{
			CTGitPathList tgpl;
			...
			if (!commonRoot.IsEmpty())
				commonRoot += L'/';
			if (old.GetFileOrDirectoryName() == GitPath->GetFileOrDirectoryName() && !old.GetContainingDirectory().GetGitPathString().IsEmpty() && GitPath->GetContainingDirectory().GetGitPathString())
				entryname = commonRoot + L'{' + GitPath->GetGitOldPathString().Mid(commonRoot.GetLength(), old.GetGitPathString().GetLength() - commonRoot.GetLength() - old.GetFileOrDirectoryName().GetLength() - 1) + L" => " + GitPath->GetGitPathString().Mid(commonRoot.GetLength(), GitPath->GetGitPathString().GetLength() - commonRoot.GetLength() - old.GetFileOrDirectoryName().GetLength() - 1) +  L"}/" + old.GetFileOrDirectoryName();
			else if (!commonRoot.IsEmpty())
				entryname = commonRoot + L'{' + GitPath->GetGitOldPathString().Mid(commonRoot.GetLength()) + L" => " + GitPath->GetGitPathString().Mid(commonRoot.GetLength()) + L'}';
			else
				entryname = GitPath->GetGitOldPathString().Mid(commonRoot.GetLength()) + L" => " + GitPath->GetGitPathString().Mid(commonRoot.GetLength());
		}
	}
...
bool CGitStatusListCtrl::BuildStatistics()
{
	...
	for (size_t i = 0; i < m_arStatusArray.size(); ++i)
	{
		...
		m_nLineAdded += _wtol(((CTGitPath*)m_arStatusArray[i])->m_StatAdd);
		m_nLineDeleted += _wtol(((CTGitPath*)m_arStatusArray[i])->m_StatDel);
...
//				}
					...
					while (pos)
					{
						...
						else
						{
							if ((selectedFilepath->m_ParentNo & (PARENT_MASK | MERGE_MASK)) == 0)
								CAppUtils::StartShowUnifiedDiff(m_hWnd, *selectedFilepath, m_CurrentVersion + L"~1", *selectedFilepath, m_CurrentVersion);
							else
							{
								CString str;
								if (!(selectedFilepath->m_ParentNo & MERGE_MASK))
									str.Format(L"%s^%d", (LPCWSTR)m_CurrentVersion, (selectedFilepath->m_ParentNo & PARENT_MASK) + 1);
			...
			case IDGITLC_LOGSUBMODULE:
				{
					CString sCmd;
					sCmd.Format(L"/command:log /path:\"%s\"", (LPCWSTR)g_Git.CombinePath(filepath));
						...
						else
						{
							...
							for (int i = 0 ; i < targetList.GetCount(); ++i)
							{
								...
								for (int nItem=0; nItem<nListboxEntries; ++nItem)
								{
									...
									{
										CString sCmd;
										sCmd.Format(L"/command:revert /path:\"%s\"", (LPCWSTR)path->GetGitPathString());
			...
			default:
				{
					...
					if (!sChangelist.IsEmpty())
					{
						...
						if (git.AddToChangeList(changelistItems, sChangelist, git_depth_empty))
						{
							...
							CMessageBox::Show(m_hWnd, git.GetLastErrorMessage(), L"TortoiseGit", MB_ICONERROR);
					}
					SetRedraw(TRUE);
				}
				break;
...
void CGitStatusListCtrl::SetGitIndexFlagsForSelectedFiles(UINT message, BOOL assumevalid, BOOL skipworktree)
{
	...
	if (!repository)
	{
		CMessageBox::Show(m_hWnd, g_Git.GetLibGit2LastErr(), L"TortoiseGit", MB_ICONERROR);
		return;
	}
	...
	else
	{
		...
		else if (file1.m_Action & file1.LOGACTIONS_DELETED)
		{
			if (file1.m_ParentNo > 0)
				fromwhere.Format(L"%s^%d", (LPCWSTR)m_CurrentVersion, file1.m_ParentNo + 1);
		...
		else
		{
			if( file1.m_ParentNo & MERGE_MASK)
			{
				...
				{
					CString str;
					str.Format(L"%s^%d", (LPCWSTR)this->m_CurrentVersion, parent1 + 1);
				...
				{
					CString str;
					str.Format(L"%s^%d", (LPCWSTR)this->m_CurrentVersion, parent2 + 1);
				...
				{
					CString cmd, output;
					cmd.Format(L"git.exe merge-base %s^%d %s^%d", (LPCWSTR)this->m_CurrentVersion, parent1 + 1,
				...
				CAppUtils::StartExtMerge(base, theirs, mine, merge, L"BASE", L"REMOTE", L"LOCAL");
			}
			else
			{
				CString str;
	...
	if(simple)
	{
		sToolTip.Format(IDS_STATUSLIST_STATUSLINE1,
			this->m_nLineAdded,this->m_nLineDeleted,
				(LPCWSTR)sModified, m_nModified,
				(LPCWSTR)sAdded, m_nAdded,
				(LPCWSTR)sDeleted, m_nDeleted,
				(LPCWSTR)sRenamed, m_nRenamed
			);
	}
	else
	{
		sToolTip.Format(IDS_STATUSLIST_STATUSLINE2,
			this->m_nLineAdded,this->m_nLineDeleted,
				(LPCWSTR)sNormal, m_nNormal,
				(LPCWSTR)sUnversioned, m_nUnversioned,
				(LPCWSTR)sModified, m_nModified,
				(LPCWSTR)sAdded, m_nAdded,
				(LPCWSTR)sDeleted, m_nDeleted,
				(LPCWSTR)sConflicted, m_nConflicted
			);
	}
	CString sStats;
	...
	while ( (index = GetNextSelectedItem(pos)) >= 0 )
	{
		...
		if (!this->m_CurrentVersion.IsEmpty() && this->m_CurrentVersion != GIT_REV_ZERO)
		{
			if (path->m_Action & CTGitPath::LOGACTIONS_DELETED)
				version.Format(L"%s^%d", (LPCWSTR)m_CurrentVersion, (path->m_ParentNo + 1) & PARENT_MASK);
			else
		...
		else
		{
			...
			tempFile = tempDir + L'\\' + path->GetWinPathString();
			CString tempSubDir = tempDir + L'\\' + path->GetContainingDirectory().GetWinPathString();
			CPathUtils::MakeSureDirectoryPathExists(tempSubDir);
			if (g_Git.GetOneFile(version, *path, tempFile))
			{
				CString out;
				out.Format(IDS_STATUSLIST_CHECKOUTFILEFAILED, (LPCWSTR)path->GetGitPathString(), (LPCWSTR)version, (LPCWSTR)tempFile);
				MessageBox(g_Git.GetGitLastErr(out, CGit::GIT_CMD_GETONEFILE), L"TortoiseGit", MB_OK | MB_ICONERROR);
				return;
			}
		}
...
bool CGitStatusListCtrl::PrepareGroups(bool bForce /* = false */)
{
	...
	if(bHasGroups)
	{
		...
		if(max >0)
		{
			wcsncpy_s(groupname, 1024, (LPCWSTR)CString(MAKEINTRESOURCE(IDS_STATUSLIST_GROUP_MERGEDFILES)), 1023);
			...
			for (int i = 0; i <= max; ++i)
			{
				...
				{
					CString rev;
					rev.Format(L"%s^%d", (LPCWSTR)m_CurrentVersion, i + 1);
					CGitHash hash;
		...
		{
			wcsncpy_s(groupname, 1024, (LPCWSTR)CString(MAKEINTRESOURCE(IDS_STATUSLIST_GROUP_MODIFIEDFILES)), 1023);
			grp.pszHeader = groupname;
			...
			{
				wcsncpy_s(groupname, 1024, (LPCWSTR)CString(MAKEINTRESOURCE(IDS_STATUSLIST_GROUP_NOTVERSIONEDFILES)), 1023);
				grp.pszHeader = groupname;
				grp.iGroupId = groupindex;
				grp.uAlign = LVGA_HEADER_LEFT;
				InsertGroup(groupindex++, &grp);
			...
			{
				wcsncpy_s(groupname, 1024, (LPCWSTR)CString(MAKEINTRESOURCE(IDS_STATUSLIST_GROUP_IGNOREDFILES)), 1023);
				grp.pszHeader = groupname;
				grp.iGroupId = groupindex;
				grp.uAlign = LVGA_HEADER_LEFT;
				InsertGroup(groupindex++, &grp);
			...
			{
				wcsncpy_s(groupname, 1024, (LPCWSTR)CString(MAKEINTRESOURCE(IDS_STATUSLIST_GROUP_IGNORELOCALCHANGES)), 1023);
				grp.pszHeader = groupname;
				grp.iGroupId = groupindex;
				grp.uAlign = LVGA_HEADER_LEFT;
				InsertGroup(groupindex++, &grp);
	...
	wcsncpy_s(groupname, 1024, (LPCWSTR)sUnassignedName, 1023);
	grp.pszHeader = groupname;
	grp.iGroupId = groupindex;
	grp.uAlign = LVGA_HEADER_LEFT;
	InsertGroup(groupindex++, &grp);
...
int CGitStatusListCtrl::UpdateUnRevFileList(CTGitPathList *List)
{
	...
	{
		CMessageBox::Show(GetSafeHwnd(), L"Failed to get UnRev file list\n" + err, L"TortoiseGit", MB_OK | MB_ICONERROR);
		return -1;
	}
...
int CGitStatusListCtrl::UpdateIgnoreFileList(CTGitPathList *List)
{
	...
	{
		CMessageBox::Show(GetSafeHwnd(), L"Failed to get Ignore file list\n" + err, L"TortoiseGit", MB_OK | MB_ICONERROR);
		return -1;
	}
			...
			if ((m_pSVNStatusListCtrl->IsGroupViewEnabled())&&(m_pSVNStatusListCtrl->GetGroupFromPoint(&clientpoint) >= 0))
			{
				...
				if (!sChangelist.IsEmpty())
				{
					...
					if (git.AddToChangeList(changelistItems, sChangelist, git_depth_empty))
					{
						...
						CMessageBox::Show(m_pSVNStatusListCtrl->m_hWnd, git.GetLastErrorMessage(), L"TortoiseGit", MB_ICONERROR);
				}
				else
				{
					SVN git;
...
void CGitStatusListCtrl::FilesExport()
{
	...
	while ((index = GetNextSelectedItem(pos)) >= 0)
	{
		...
		else
		{
			...
			{
				CString out;
				out.Format(IDS_STATUSLIST_CHECKOUTFILEFAILED, (LPCWSTR)fd->GetGitPathString(), (LPCWSTR)m_CurrentVersion, (LPCWSTR)filename);
...
{
	CString filename;
	filename.Format(L"%s-%s%s", (LPCWSTR)path->GetBaseFilename(), (LPCWSTR)this->m_CurrentVersion.Left(g_Git.GetShortHASHLength()), (LPCWSTR)path->GetFileExtension());
	...
	if (ret == IDOK)
	{
		...
		else
		{
			...
			{
				out.Format(IDS_STATUSLIST_CHECKOUTFILEFAILED, (LPCWSTR)path->GetGitPathString(), (LPCWSTR)m_CurrentVersion, (LPCWSTR)filename);
...
int CGitStatusListCtrl::RevertSelectedItemToVersion(bool parent)
{
	...
	while ((index = GetNextSelectedItem(pos)) >= 0)
	{
		...
		if (parent)
		{
			...
			CString ref;
			ref.Format(L"%s^%d", (LPCWSTR)m_CurrentVersion, parentNo + 1);
			CGitHash hash;
		...
		cmd.Format(L"git.exe checkout %s -- \"%s\"", (LPCWSTR)version, (LPCWSTR)filename);
		out.Empty();
	...
	for (auto it = versionMap.cbegin(); it != versionMap.cend(); ++it)
	{
		...
		versionEntry.Format(IDS_STATUSLIST_FILESREVERTED, it->second, (LPCWSTR)it->first);
		out += versionEntry + L"\r\n";
	}
	if (!out.IsEmpty())
		CMessageBox::Show(nullptr, out, L"TortoiseGit", MB_OK);
	return 0;
}
...
void CGitStatusListCtrl::OpenFile(CTGitPath*filepath,int mode)
{
	...
	else
	{
		...
		file.Format(L"%s\\%s_%s%s",
					(LPCWSTR)temp,
					(LPCWSTR)filepath->GetBaseFilename(),
					(LPCWSTR)m_CurrentVersion.Left(g_Git.GetShortHASHLength()),
					(LPCWSTR)filepath->GetFileExtension());
		CString cmd,out;
		if(g_Git.GetOneFile(m_CurrentVersion, *filepath, file))
		{
			out.Format(IDS_STATUSLIST_CHECKOUTFILEFAILED, (LPCWSTR)filepath->GetGitPathString(), (LPCWSTR)m_CurrentVersion, (LPCWSTR)file);
			CMessageBox::Show(nullptr, g_Git.GetGitLastErr(out, CGit::GIT_CMD_GETONEFILE), L"TortoiseGit", MB_OK);
			return;
		}
