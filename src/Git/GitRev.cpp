...
{
	if (refname.GetLength() >= 8 && refname.Find(L"00000000") == 0)
	{
		Clear();
		m_Subject = L"Working Copy";
		return 0;
	}
...
{
	ATLTRACE(L"Commit %s\r\n", (LPCWSTR)this->m_CommitHash.ToString());
	for (unsigned int i = 0; i < this->m_ParentHash.size(); ++i)
	{
		ATLTRACE(L"Parent %i %s", i, (LPCWSTR)m_ParentHash[i].ToString());
	}
	ATLTRACE(L"\n");
}
