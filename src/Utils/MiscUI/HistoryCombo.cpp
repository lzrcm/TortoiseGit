...
int CHistoryCombo::InsertEntry(const CString& combostring, INT_PTR pos)
{
	...
	if (m_bURLHistory)
	{
		...
		{
			if (combostring.Left(5) == L"http:")
				cbei.iImage = SYS_IMAGE_LIST().GetFileIconIndex(L".html");
			else if (combostring.Left(6) == L"https:")
				cbei.iImage = SYS_IMAGE_LIST().GetFileIconIndex(L".html");
			else if (combostring.Left(5) == L"file:")
				cbei.iImage = SYS_IMAGE_LIST().GetDirIconIndex();
			else if (combostring.Left(4) == L"git:")
				cbei.iImage = m_nGitIconIndex;
			else if (combostring.Left(4) == L"ssh:")
				cbei.iImage = m_nGitIconIndex;
			else
				cbei.iImage = SYS_IMAGE_LIST().GetDirIconIndex();
		}
