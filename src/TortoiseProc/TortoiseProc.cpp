...
BOOL CTortoiseProcApp::InitInstance()
{
...

	{
		...
		versionString.Format(L"%d", CGit::ms_LastMsysGitVersion);
	...
	if (parser.HasKey(L"urlhandler"))
	{
		...
		if (url.Find(L"tgit://clone/") == 0)
			url = url.Mid(13); // 21 = "tgit://clone/".GetLength()
		else if (url.Find(L"github-windows://openRepo/") == 0)
		{
		...
		else if (url.Find(L"smartgit://cloneRepo/") == 0)
