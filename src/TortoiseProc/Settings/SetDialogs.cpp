	...
	, m_bFullCommitMessageOnLogLine(FALSE)
{
	...
	m_regFontName = CRegString(L"Software\\TortoiseGit\\LogFontName", L"Courier New");
	m_regFontSize = CRegDWORD(L"Software\\TortoiseGit\\LogFontSize", 8);
...
void CSetDialogs::OnCbnSelchangeDefaultlogscale()
{
	...
	if (sel > 1 && (m_sDefaultLogs.IsEmpty() || _wtol((LPCWSTR)m_sDefaultLogs) == 0))
		m_sDefaultLogs.Format(L"%ld", (DWORD)m_regDefaultLogs);
	else if (sel <= 1)
		m_sDefaultLogs.Empty();
...
BOOL CSetDialogs::OnApply()
{
	...
	int val = _wtol((LPCWSTR)m_sDefaultLogs);
	if (sel > 1 && val > 0)
