...
void CCommitDlg::OnOK()
{
	...
	for (int i = 0; i < nListItems && !m_bCommitMessageOnly; ++i)
	{
		...
		{
			CString message;
			message.Format(IDS_COMMITDLG_SUBMODULEDIRTY, (LPCWSTR)entry->GetGitPathString());
			...
			{
				CString cmdCommit;
				cmdCommit.Format(L"/command:commit /path:\"%s\\%s\"", (LPCWSTR)g_Git.m_CurrentDir, entry->GetWinPath());
	...
	if (g_Git.UsingLibGit2(CGit::GIT_CMD_COMMIT_UPDATE_INDEX))
	{
		...
		do
		{
			...
			{
				CMessageBox::Show(m_hWnd, CGit::GetLibGit2LastErr(L"Could not get last commit."), L"TortoiseGit", MB_OK | MB_ICONERROR);
				break;
			}
			...
			{
				CMessageBox::Show(m_hWnd, CGit::GetLibGit2LastErr(L"Could not read tree of commit."), L"TortoiseGit", MB_OK | MB_ICONERROR);
				break;
			}
			...
			{
				CMessageBox::Show(m_hWnd, CGit::GetLibGit2LastErr(L"Could not get the repository index."), L"TortoiseGit", MB_OK | MB_ICONERROR);
				break;
			}
			...
			{
				CMessageBox::Show(m_hWnd, CGit::GetLibGit2LastErr(L"Could not read the tree into the index."), L"TortoiseGit", MB_OK | MB_ICONERROR);
				break;
			}
			...
			for (int j = 0; j < nListItems; ++j)
			{
				...
				if (entry->m_Checked && !m_bCommitMessageOnly)
				{
					...
					else
					{
						...
						{
							bAddSuccess = false;
							CMessageBox::Show(m_hWnd, CGit::GetLibGit2LastErr(L"Could not add \"" + entry->GetGitPathString() + L"\" to index."), L"TortoiseGit", MB_OK | MB_ICONERROR);
							break;
						}
					}
				...
				else
				{
					if (entry->m_Action & CTGitPath::LOGACTIONS_ADDED || entry->m_Action & CTGitPath::LOGACTIONS_REPLACED)
					{
						...
						if (entry->m_Action & CTGitPath::LOGACTIONS_REPLACED && !entry->GetGitOldPathString().IsEmpty())
						{
							...
							{
								bAddSuccess = false;
								CMessageBox::Show(m_hWnd, CGit::GetLibGit2LastErr(L"Could not reset \"" + entry->GetGitOldPathString() + L"\" to old index entry."), L"TortoiseGit", MB_OK | MB_ICONERROR);
								break;
							}
					...
					else if (!(entry->m_Action & CTGitPath::LOGACTIONS_UNVER))
					{
						...
						{
							bAddSuccess = false;
							CMessageBox::Show(m_hWnd, CGit::GetLibGit2LastErr(L"Could not reset \"" + entry->GetGitPathString() + L"\" to old index entry."), L"TortoiseGit", MB_OK | MB_ICONERROR);
							break;
						}
	...
	if (m_BugTraqProvider)
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			if (FAILED(hr = pProvider2->CheckCommit(GetSafeHwnd(), parameters, repositoryRoot, commonRoot, pathList, commitMessage, &temp)))
			{
				COMError ce(hr);
				CString sErr;
				sErr.Format(IDS_ERR_FAILEDISSUETRACKERCOM, (LPCWSTR)m_bugtraq_association.GetProviderName(), ce.GetMessageAndDescription().c_str());
				CMessageBox::Show(m_hWnd, sErr, L"TortoiseGit", MB_ICONERROR);
			}
			else
			{
				CString sError = temp;
				if (!sError.IsEmpty())
				{
					CMessageBox::Show(m_hWnd, sError, L"TortoiseGit", MB_ICONERROR);
	...
	if (m_bCommitMessageOnly || bAddSuccess && (nchecked || m_bCommitAmend ||  CTGitPath(g_Git.m_CurrentDir).IsMergeActive()))
	{
		...
		{
			CMessageBox::Show(nullptr, L"Could not save commit message", L"TortoiseGit", MB_OK | MB_ICONERROR);
			InterlockedExchange(&m_bBlock, FALSE);
			return;
		}
		...
		if (m_bSetCommitDateTime)
		{
			...
			if (m_bCommitAmend && m_AsCommitDateCtrl.GetCheck())
				...
				dateTime.Format(L"--date=%sT%s", (LPCWSTR)date.Format(L"%Y-%m-%d"), (LPCWSTR)time.Format(L"%H:%M:%S"));
		}
		CString author;
		if (m_bSetAuthor)
			author.Format(L"--author=\"%s\"", (LPCWSTR)m_sAuthor);
		...
		cmd.Format(L"git.exe commit %s %s %s %s -F \"%s\"", (LPCWSTR)author, (LPCWSTR)dateTime, (LPCWSTR)amend, (LPCWSTR)allowEmpty, (LPCWSTR)tempfile);
...
{
	CString reg;
	reg.Format(L"Software\\TortoiseGit\\History\\commit%s", (LPCWSTR)m_ListCtrl.m_sUUID);
	reg.Replace(L':', L'_');
...
UINT CCommitDlg::StatusThread()
{
		 ...
		 && !PathFileExists(dotGitPath + L"MERGE_HEAD"))
	{
		...
		if (CMessageBox::ShowCheck(m_hWnd, temp, L"TortoiseGit", MB_ICONINFORMATION | MB_YESNO, L"NothingToCommitShowUnversioned", nullptr) == IDYES)
		{
...
void CCommitDlg::ParseSnippetFile(const CString& sFile, std::map<CString, CString>& mapSnippet)
{
	...
	try
	{
		...
		while (m_bRunThread && file.ReadString(strLine))
		{
			if (strLine.IsEmpty())
				continue;
			if (strLine.Left(1) == L'#') // comment char
				continue;
			int eqpos = strLine.Find('=');
	...
	if (cmd == m_nPopupPasteListCmd)
	{
		...
		for (int i=0; i<nListItems; ++i)
		{
			...
			if (entry&&entry->m_Checked)
			{
				...
				logmsg.AppendFormat(L"%-10s %s\r\n", (LPCWSTR)status, (LPCWSTR)m_ListCtrl.GetItemText(i, 0));
			}
		}
...
void CCommitDlg::OnBnClickedBugtraqbutton()
{
	...
	if (SUCCEEDED(hr))
	{
		...
		else
		{
			...
			m_cLogMessage.SetText((LPCWSTR)temp);
...
//					}
	...
	else
	{
		...
		{
			CString sErr;
			sErr.Format(IDS_ERR_FAILEDISSUETRACKERCOM, (LPCWSTR)m_bugtraq_association.GetProviderName(), _com_error(hr).ErrorMessage());
			CMessageBox::Show(m_hWnd, sErr, L"TortoiseGit", MB_ICONERROR);
			return;
		}
		...
		while(pos)
		{
			...
			if(p && !(p->m_Action&CTGitPath::LOGACTIONS_UNVER) )
			{
				...
				if(m_bCommitAmend==TRUE && m_bAmendDiffToLastCommit==FALSE)
					head = L"HEAD~1";
				cmd.Format(L"git.exe diff %s -- \"%s\"", (LPCWSTR)head, (LPCWSTR)p->GetGitPathString());
...
CString CCommitDlg::GetSignedOffByLine()
{
	...
	str.Format(L"Signed-off-by: %s <%s>", (LPCWSTR)username, (LPCWSTR)email);
...
void CCommitDlg::OnBnClickedCommitAmend()
{
	...
	{
		GitRev rev;
		if (rev.GetCommit(CString(L"HEAD")))
			MessageBox(rev.GetLastErr(), L"TortoiseGit", MB_ICONERROR);
...
void CCommitDlg::OnBnClickedCommitSetauthor()
{
	...
	if (m_bSetAuthor)
	{
		m_sAuthor.Format(L"%s <%s>", (LPCWSTR)g_Git.GetUserName(), (LPCWSTR)g_Git.GetUserEmail());
		if (m_bCommitAmend)
		{
			GitRev headRevision;
			if (headRevision.GetCommit(L"HEAD"))
				MessageBox(headRevision.GetLastErr(), L"TortoiseGit", MB_ICONERROR);
			m_sAuthor.Format(L"%s <%s>", (LPCWSTR)headRevision.GetAuthorName(), (LPCWSTR)headRevision.GetAuthorEmail());
		}
