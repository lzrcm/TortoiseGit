...
bool CAppUtils::StashApply(CString ref, bool showChanges /* true */)
{
	...
	if (ref.Find(L"refs/") == 0)
		ref = ref.Mid(5);
	if (ref.Find(L"stash{") == 0)
	...
	else
	{
		...
		{
			CMessageBox::Show(nullptr, message ,L"TortoiseGit", MB_OK | MB_ICONINFORMATION);
			return true;
		}
	}
	return false;
...
bool CAppUtils::StashPop(int showChanges /* = 1 */)
{
	...
	else
	{
		...
		{
			if (CMessageBox::Show(nullptr, CString(message + L'\n' + CString(MAKEINTRESOURCE(IDS_SEECHANGES))), L"TortoiseGit", MB_YESNO | MB_ICONINFORMATION) == IDYES)
			{
				CChangedDlg dlg;
				dlg.m_pathList.AddPath(CTGitPath());
				dlg.DoModal();
...
{
	DWORD length = 0;
	CAutoFile hFile = ::CreateFile(sDiffPath.GetWinPath(), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, FLLE, nullptr);
	if (!hFile)
		return TRUE;
...
void CAppUtils::CreateFontForLogs(CFont& fontToCreate)
{
	...
	logFont.lfHeight			= -MulDiv((DWORD)CRegDWORD(L"Software\\TortoiseGit\\LogFontSize", 8), GetDeviceCaps(hScreenDC, LOGPIXELSY), 72);
	...
	wcscpy_s(logFont.lfFaceName, 32, (LPCWSTR)(CString)CRegString(L"Software\\TortoiseGit\\LogFontName", L"Courier New"));
...
bool CAppUtils::LaunchPAgent(const CString* keyfile, const CString* pRemote)
{
	...
	if (!keyfile)
	{
		cmd.Format(L"remote.%s.puttykeyfile", (LPCWSTR)remote);
												...
												bool bNoPrefix)
{
	...
	if (GetMsysgitVersion() > 0x01080100)
		diffContext = g_Git.GetConfigValueInt32(L"diff.context", -1);
	CString tempfile=GetTempFile();
	if (g_Git.GetUnifiedDiff(url1, rev1, rev2, tempfile, bMerge, bCombine, diffContext, bNoPrefix))
	{
		CMessageBox::Show(hWnd, g_Git.GetGitLastErr(L"Could not get unified diff.", CGit::GIT_CMD_DIFF), L"TortoiseGit", MB_OK);
		return false;
	}
	CAppUtils::StartUnifiedDiffViewer(tempfile, rev1 + L":" + rev2);
...
bool CAppUtils::Export(const CString* BashHash, const CTGitPath* orgPath)
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		cmd.Format(L"git.exe archive --output=\"%s\" --format=zip --verbose %s --",
					(LPCWSTR)dlg.m_strFile, (LPCWSTR)g_Git.FixBranchName(dlg.m_VersionName));
...
bool CAppUtils::UpdateBranchDescription(const CString& branch, CString description)
{
	...
	CString key;
	key.Format(L"branch.%s.description", (LPCWSTR)branch);
...
bool CAppUtils::CreateBranchTag(bool isTag /*true*/, const CString* commitHash /*nullptr*/, bool switchNewBranch /*false*/, LPCWSTR name /*nullptr*/)
{
	...
	if(dlg.DoModal()==IDOK)
	{
		...
		if (isTag)
		{
			...
			cmd.Format(L"git.exe tag %s %s %s %s",
				(LPCWSTR)force,
				(LPCWSTR)sign,
				(LPCWSTR)dlg.m_BranchTagName,
				(LPCWSTR)g_Git.FixBranchName(dlg.m_VersionName)
				);
			...
			if(!dlg.m_Message.Trim().IsEmpty())
			{
				...
				{
					CMessageBox::Show(nullptr, L"Could not save tag message", L"TortoiseGit", MB_OK | MB_ICONERROR);
					return FALSE;
				}
				cmd += L" -F " + tempfile;
			}
		...
		{
			CMessageBox::Show(nullptr, out, L"TortoiseGit", MB_OK | MB_ICONERROR);
			return FALSE;
		}
...
bool CAppUtils::Switch(const CString& initialRefName)
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		if (dlg.m_VersionName.Left(11) == L"refs/heads/" && dlg.m_bBranchOverride != TRUE)
...
bool CAppUtils::PerformSwitch(const CString& ref, bool bForce /* false */, const CString& sNewBranch /* CString() */, bool bBranchOverride /* false */, BOOL bTrack /* 2 */, bool bMerge /* false */)
{
	...
	if(!sNewBranch.IsEmpty()){
		if (bBranchOverride)
			branch.Format(L"-B %s ", (LPCWSTR)sNewBranch);
		else
			branch.Format(L"-b %s ", (LPCWSTR)sNewBranch);
		if (bTrack == TRUE)
			track = L"--track ";
	...
	cmd.Format(L"git.exe checkout %s%s%s%s%s --",
		 ...
		 (LPCWSTR)merge,
		 (LPCWSTR)branch,
		 (LPCWSTR)g_Git.FixBranchName(ref));
	...
	progress.m_PostCmdCallback = [&](DWORD status, PostCmdList& postCmdList)
	{
		if (!status)
		{
			...
			if (gitPath.HasSubmodules())
			{
				...
				{
					CString sCmd;
					sCmd.Format(L"/command:subupdate /bkpath:\"%s\"", (LPCWSTR)g_Git.m_CurrentDir);
		...
		else
		{
			if (bMerge && g_Git.HasWorkingTreeConflicts() > 0)
			{
				...
				{
					CString sCmd;
					sCmd.Format(L"/command:commit /path:\"%s\"", (LPCWSTR)g_Git.m_CurrentDir);
...
bool CAppUtils::OpenIgnoreFile(CIgnoreFile &file, const CString& filename)
{
	...
	{
		CMessageBox::Show(nullptr, filename + L" Open Failure", L"TortoiseGit", MB_OK | MB_ICONERROR);
		return false;
	}
...
static bool Reset(const CString& resetTo, int resetType)
{
	...
	cmd.Format(L"git.exe reset %s %s --", (LPCWSTR)type, (LPCWSTR)resetTo);
...
bool CAppUtils::ConflictEdit(const CTGitPath& path, bool /*bAlternativeTool = false*/, bool revertTheirMy /*= false*/, HWND resolveMsgHwnd /*= nullptr*/)
{
	...
	CString cmd;
	cmd.Format(L"git.exe ls-files -u -t -z -- \"%s\"", (LPCWSTR)merge.GetGitPathString());
	...
	{
		CMessageBox::Show(nullptr, L"Parse ls-files failed!", L"TortoiseGit", MB_OK | MB_ICONERROR);
		return FALSE;
	}
	...
	for (int i = 0; i< list.GetCount(); ++i)
	{
		...
		if( list[i].m_Stage == 1)
		{
			cmd.Format(format, list[i].m_Stage, (LPCWSTR)list[i].GetGitPathString());
			b_base = true;
	...
	if(b_local && b_remote )
	{
		...
		if( revertTheirMy )
			bRet = !!CAppUtils::StartExtMerge(base, mine, theirs, merge, L"BASE", L"REMOTE", L"LOCAL", CString(), false, resolveMsgHwnd, true);
		else
			bRet = !!CAppUtils::StartExtMerge(base, theirs, mine, merge, L"BASE", L"REMOTE", L"LOCAL", CString(), false, resolveMsgHwnd, true);
	}
	else
	{
		...
		if(dlg.DoModal() == IDOK)
		{
			CString out;
			if(dlg.m_bIsDelete)
				cmd.Format(L"git.exe rm -- \"%s\"", (LPCWSTR)merge.GetGitPathString());
			else
				cmd.Format(L"git.exe add -- \"%s\"", (LPCWSTR)merge.GetGitPathString());
			...
			else if (pInfoData->treeconflict_kind == svn_wc_conflict_kind_tree)
			{
				...
				if (pInfoData->treeconflict_nodekind == svn_node_file)
				{
					...
					case svn_wc_operation_update:
						...
						case svn_wc_conflict_action_edit:
							sConflictAction.Format(IDS_TREECONFLICT_FILEUPDATEEDIT, (LPCWSTR)sItemName);
							sResolveTheirs.LoadString(IDS_TREECONFLICT_RESOLVE_KEEPREPOSITORYFILE);
							break;
						case svn_wc_conflict_action_add:
							sConflictAction.Format(IDS_TREECONFLICT_FILEUPDATEADD, (LPCWSTR)sItemName);
							sResolveTheirs.LoadString(IDS_TREECONFLICT_RESOLVE_KEEPREPOSITORYFILE);
							break;
						case svn_wc_conflict_action_delete:
							sConflictAction.Format(IDS_TREECONFLICT_FILEUPDATEDELETE, (LPCWSTR)sItemName);
					...
					case svn_wc_operation_switch:
						...
						{
						case svn_wc_conflict_action_edit:
							sConflictAction.Format(IDS_TREECONFLICT_FILESWITCHEDIT, (LPCWSTR)sItemName);
							sResolveTheirs.LoadString(IDS_TREECONFLICT_RESOLVE_KEEPREPOSITORYFILE);
							break;
						case svn_wc_conflict_action_add:
							sConflictAction.Format(IDS_TREECONFLICT_FILESWITCHADD, (LPCWSTR)sItemName);
							sResolveTheirs.LoadString(IDS_TREECONFLICT_RESOLVE_KEEPREPOSITORYFILE);
							break;
						case svn_wc_conflict_action_delete:
							sConflictAction.Format(IDS_TREECONFLICT_FILESWITCHDELETE, (LPCWSTR)sItemName);
							sResolveTheirs.LoadString(IDS_TREECONFLICT_RESOLVE_REMOVEFILE);
							break;
						}
						break;
					case svn_wc_operation_merge:
						...
						{
						case svn_wc_conflict_action_edit:
							sConflictAction.Format(IDS_TREECONFLICT_FILEMERGEEDIT, (LPCWSTR)sItemName);
							sResolveTheirs.LoadString(IDS_TREECONFLICT_RESOLVE_KEEPREPOSITORYFILE);
							break;
						case svn_wc_conflict_action_add:
							sResolveTheirs.Format(IDS_TREECONFLICT_FILEMERGEADD, (LPCWSTR)sItemName);
							sResolveTheirs.LoadString(IDS_TREECONFLICT_RESOLVE_KEEPREPOSITORYFILE);
							break;
						case svn_wc_conflict_action_delete:
							sConflictAction.Format(IDS_TREECONFLICT_FILEMERGEDELETE, (LPCWSTR)sItemName);
							sResolveTheirs.LoadString(IDS_TREECONFLICT_RESOLVE_REMOVEFILE);
							break;
						}
						break;
				...
				else if (pInfoData->treeconflict_nodekind == svn_node_dir)
				{
					...
					case svn_wc_operation_update:
						...
						{
						case svn_wc_conflict_action_edit:
							sConflictAction.Format(IDS_TREECONFLICT_DIRUPDATEEDIT, (LPCWSTR)sItemName);
							sResolveTheirs.LoadString(IDS_TREECONFLICT_RESOLVE_KEEPREPOSITORYDIR);
							break;
						case svn_wc_conflict_action_add:
							sConflictAction.Format(IDS_TREECONFLICT_DIRUPDATEADD, (LPCWSTR)sItemName);
							sResolveTheirs.LoadString(IDS_TREECONFLICT_RESOLVE_KEEPREPOSITORYDIR);
							break;
						case svn_wc_conflict_action_delete:
							sConflictAction.Format(IDS_TREECONFLICT_DIRUPDATEDELETE, (LPCWSTR)sItemName);
					...
					case svn_wc_operation_switch:
						...
						{
						case svn_wc_conflict_action_edit:
							sConflictAction.Format(IDS_TREECONFLICT_DIRSWITCHEDIT, (LPCWSTR)sItemName);
							sResolveTheirs.LoadString(IDS_TREECONFLICT_RESOLVE_KEEPREPOSITORYDIR);
							break;
						case svn_wc_conflict_action_add:
							sConflictAction.Format(IDS_TREECONFLICT_DIRSWITCHADD, (LPCWSTR)sItemName);
							sResolveTheirs.LoadString(IDS_TREECONFLICT_RESOLVE_KEEPREPOSITORYDIR);
							break;
						case svn_wc_conflict_action_delete:
							sConflictAction.Format(IDS_TREECONFLICT_DIRSWITCHDELETE, (LPCWSTR)sItemName);
							sResolveTheirs.LoadString(IDS_TREECONFLICT_RESOLVE_REMOVEDIR);
							break;
						}
						break;
					case svn_wc_operation_merge:
						...
						{
						case svn_wc_conflict_action_edit:
							sConflictAction.Format(IDS_TREECONFLICT_DIRMERGEEDIT, (LPCWSTR)sItemName);
							sResolveTheirs.LoadString(IDS_TREECONFLICT_RESOLVE_KEEPREPOSITORYDIR);
							break;
						case svn_wc_conflict_action_add:
							sConflictAction.Format(IDS_TREECONFLICT_DIRMERGEADD, (LPCWSTR)sItemName);
							sResolveTheirs.LoadString(IDS_TREECONFLICT_RESOLVE_KEEPREPOSITORYDIR);
							break;
						case svn_wc_conflict_action_delete:
							sConflictAction.Format(IDS_TREECONFLICT_DIRMERGEDELETE, (LPCWSTR)sItemName);
							sResolveTheirs.LoadString(IDS_TREECONFLICT_RESOLVE_REMOVEDIR);
							break;
						}
						break;
				...
				sConflictReason.Format(uReasonID, (LPCWSTR)sConflictAction);
...
bool CAppUtils::IsSSHPutty()
{
	...
	if(sshclient.Find(L"plink.exe", 0) >= 0)
		return true;
	return false;
}
...
CString CAppUtils::GetClipboardLink(const CString &skipGitPrefix, int paramsCount)
{
	...
	if(!sClipboardText.IsEmpty())
	{
		...
		if(sClipboardText.Find( L"https://") == 0)
			return sClipboardText;

		if(sClipboardText.Find( L"git://") == 0)
			return sClipboardText;

		if(sClipboardText.Find( L"ssh://") == 0)
			return sClipboardText;
...
bool CAppUtils::Pull(bool showPush, bool showStashPop)
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		cmd.Format(L"git.exe pull --progress%s -v %s%s%s%s%s%s%s\"%s\" %s", CRegDWORD(L"Software\\TortoiseGit\\PullRebaseBehaviorLike1816", FALSE) == FALSE ? L" --no-rebase" : L"", (LPCWSTR)noff, (LPCWSTR)ffonly, (LPCWSTR)squash, (LPCWSTR)nocommit, (LPCWSTR)depth, (LPCWSTR)notags, (LPCWSTR)prune, (LPCWSTR)url, (LPCWSTR)dlg.m_RemoteBranchName);
		CProgressDlg progress;
		progress.m_GitCmd = cmd;
		...
		progress.m_PostCmdCallback = [&](DWORD status, PostCmdList& postCmdList)
		{
			...
			{
				postCmdList.emplace_back(IDI_PULL, IDS_MENUPULL, [&]{ Pull(); });
				postCmdList.emplace_back(IDI_COMMIT, IDS_MENUSTASHSAVE, [&]{ StashSave(L"", true); });
				return;
			}
	...
	while (true)
	{
		...
		else if (response == IDC_REBASE_POST_BUTTON + 2)
		{
			...
			cmd.Format(L"git.exe format-patch -o \"%s\" %s..%s",
				(LPCWSTR)g_Git.m_CurrentDir,
				(LPCWSTR)g_Git.FixBranchName(dlg.m_Upstream),
				(LPCWSTR)g_Git.FixBranchName(dlg.m_Branch));
...
static bool DoFetch(const CString& url, const bool fetchAllRemotes, const bool loadPuttyAgent, const int prune, const bool bDepth, const int nDepth, const int fetchTags, const CString& remoteBranch, int runRebase, const bool rebasePreserveMerges)
{
	...
	if (fetchAllRemotes)
		cmd.Format(L"git.exe fetch --all -v%s", (LPCWSTR)arg);
	else
		cmd.Format(L"git.exe fetch -v%s \"%s\" %s", (LPCWSTR)arg, (LPCWSTR)url, (LPCWSTR)remoteBranch);
...
bool CAppUtils::DoPush(bool autoloadKey, bool pack, bool tags, bool allRemotes, bool allBranches, bool force, bool forceWithLease, const CString& localBranch, const CString& remote, const CString& remoteBranch, bool setUpstream, int recurseSubmodules)
{
	...
	if (CHooks::Instance().PrePush(g_Git.m_CurrentDir, exitcode, error))
	{
		...
		{
			CString temp;
			temp.Format(IDS_ERR_HOOKFAILED, (LPCWSTR)error);
			CMessageBox::Show(nullptr, temp, L"TortoiseGit", MB_OK | MB_ICONERROR);
			return false;
		}
	}
	...
	for (unsigned int i = 0; i < remotesList.size(); ++i)
	{
		...
		if (allBranches)
		{
			cmd.Format(L"git.exe push --all %s\"%s\"",
				(LPCWSTR)arg,
				(LPCWSTR)remotesList[i]);

			if (tags)
			{
				progress.m_GitCmdList.push_back(cmd);
				cmd.Format(L"git.exe push --tags %s\"%s\"", (LPCWSTR)arg, (LPCWSTR)remotesList[i]);
			}
		}
		else
		{
		...
		{
			cmd.Format(L"git.exe rev-list --count --first-parent %s", (LPCWSTR)localBranch);
			progress.m_GitCmdList.push_back(cmd);
		}
	}
	...
	progress.m_PostCmdCallback = [&](DWORD status, PostCmdList& postCmdList)
	{
		...
		if (!superprojectRoot.IsEmpty())
		{
			...
			{
				CString sCmd;
				sCmd.Format(L"/command:commit /path:\"%s\"", (LPCWSTR)superprojectRoot);
				RunTortoiseGitProc(sCmd);
			});
		}
	};
...
bool CAppUtils::RequestPull(const CString& endrevision, const CString& repositoryUrl, bool bIsMainWnd)
{
	...
	{
		CString cmd;
		cmd.Format(L"git.exe request-pull %s \"%s\" %s", (LPCWSTR)dlg.m_StartRevision, (LPCWSTR)dlg.m_RepositoryURL, (LPCWSTR)dlg.m_EndRevision);
...
BOOL CAppUtils::Merge(const CString* commit, bool showStashPop)
{
	...
	if(dlg.DoModal()==IDOK)
	{
		...
		if (!dlg.m_MergeStrategy.IsEmpty())
		{
			...
			{
				args += L" --strategy-option=" + dlg.m_StrategyOption;
				if (!dlg.m_StrategyParam.IsEmpty())
					args += L"=" + dlg.m_StrategyParam;
			}
		}
		...
		Prodlg.m_PostCmdCallback = [&](DWORD status, PostCmdList& postCmdList)
		{
			if (status)
			{
				...
				postCmdList.emplace_back(IDI_COMMIT, IDS_MENUSTASHSAVE, [&]{ CAppUtils::StashSave(L"", false, false, true, g_Git.FixBranchName(dlg.m_VersionName)); });
				return;
			}
			...
			if (dlg.m_bIsBranch && dlg.m_VersionName.Find(L"remotes/") == -1) // do not ask to remove remote branches
			{
				postCmdList.emplace_back(IDI_DELETE, IDS_PROC_REMOVEBRANCH, [&]
				{
					...
					{
						CString cmd, out;
						cmd.Format(L"git.exe branch -D -- %s", (LPCWSTR)dlg.m_VersionName);
						if (g_Git.Run(cmd, &out, CP_UTF8))
...
int CAppUtils::ResolveConflict(CTGitPath& path, resolve_with resolveWith)
{
	...
	{
		CString cmd;
		cmd.Format(L"git.exe ls-files -u -t -z -- \"%s\"", (LPCWSTR)path.GetGitPathString());
		if (g_Git.Run(cmd, &vector))
		{
			CMessageBox::Show(nullptr, L"git ls-files failed!", L"TortoiseGit", MB_OK);
			return -1;
		}
		...
		if (list.ParserFromLsFile(vector))
		{
			CMessageBox::Show(nullptr, L"Parse ls-files failed!", L"TortoiseGit", MB_OK);
			return -1;
		}
	...
	if (resolveWith == RESOLVE_WITH_THEIRS)
	{
		...
		if (b_local && b_remote)
			gitcmd.Format(L"git.exe checkout-index -f --stage=3 -- \"%s\"", (LPCWSTR)path.GetGitPathString());
		else if (b_remote)
			gitcmd.Format(L"git.exe add -f -- \"%s\"", (LPCWSTR)path.GetGitPathString());
		else if (b_local)
			gitcmd.Format(L"git.exe rm -f -- \"%s\"", (LPCWSTR)path.GetGitPathString());
		if (g_Git.Run(gitcmd, &output, CP_UTF8))
		{
			CMessageBox::Show(nullptr, output, L"TortoiseGit", MB_ICONERROR);
			return -1;
		}
	}
	else if (resolveWith == RESOLVE_WITH_MINE)
	{
		CString gitcmd, output;
		if (b_local && b_remote)
			gitcmd.Format(L"git.exe checkout-index -f --stage=2 -- \"%s\"", (LPCWSTR)path.GetGitPathString());
		else if (b_local)
			gitcmd.Format(L"git.exe add -f -- \"%s\"", (LPCWSTR)path.GetGitPathString());
		else if (b_remote)
			gitcmd.Format(L"git.exe rm -f -- \"%s\"", (LPCWSTR)path.GetGitPathString());
