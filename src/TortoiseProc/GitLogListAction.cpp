...
int CGitLogList::RevertSelectedCommits(int parent)
{
	...
	while(pos)
	{
		...
		if (progress.IsVisible())
		{
			...
			progress.FormatNonPathLine(2, L"%s", (LPCWSTR)r1->GetSubject());
		...
		if (g_Git.GitRevert(parent, r1->m_CommitHash))
		{
			...
			if( GetSelectedCount() == 1)
				CMessageBox::Show(nullptr, str, L"TortoiseGit", MB_OK | MB_ICONERROR);
			else if (CMessageBox::Show(nullptr, str, L"TortoiseGit", 2, IDI_ERROR, CString(MAKEINTRESOURCE(IDS_SKIPBUTTON)), CString(MAKEINTRESOURCE(IDS_ABORTBUTTON))) == 2)
				return ret;
		}
		else
			ret =0;
...
int CGitLogList::CherryPickFrom(CString from, CString to)
{
	...
	CString range;
	range.Format(L"%s..%s", (LPCWSTR)from, (LPCWSTR)to);
	...
	for (int i = (int)logs.size() - 1; i >= 0; i--)
	{
		if (progress.IsVisible())
		{
			...
			progress.FormatNonPathLine(2, L"%s", (LPCWSTR)logs.GetGitRevAt(i).GetSubject());
		...
		CString cmd,out;
		cmd.Format(L"git.exe cherry-pick %s", (LPCWSTR)logs.GetGitRevAt(i).m_CommitHash.ToString());
...
int CGitLogList::DeleteRef(const CString& ref)
{
	...
	{
		CString msg;
		msg.Format(IDS_PROC_DELETEREMOTEBRANCH, (LPCWSTR)ref);
		int result = CMessageBox::Show(nullptr, msg, L"TortoiseGit", 3, IDI_QUESTION, CString(MAKEINTRESOURCE(IDS_PROC_DELETEREMOTEBRANCH_LOCALREMOTE)), CString(MAKEINTRESOURCE(IDS_PROC_DELETEREMOTEBRANCH_LOCAL)), CString(MAKEINTRESOURCE(IDS_ABORTBUTTON)));
		if (result == 1)
		{
			...
			if (g_Git.DeleteRemoteRefs(remoteName, list))
				CMessageBox::Show(nullptr, g_Git.GetGitLastErr(L"Could not delete remote ref.", CGit::GIT_CMD_PUSH), L"TortoiseGit", MB_OK | MB_ICONERROR);
			sysProgressDlg.Stop();
			return TRUE;
		}
		else if (result == 2)
		{
			...
			{
				CMessageBox::Show(nullptr, g_Git.GetGitLastErr(L"Could not delete reference.", CGit::GIT_CMD_DELETETAGBRANCH), L"TortoiseGit", MB_OK | MB_ICONERROR);
				return FALSE;
			}
			return TRUE;
		}
	...
	else if (CGit::GetShortName(ref, shortname, L"refs/stash"))
	{
		...
		int choose = CMessageBox::Show(nullptr, msg, L"TortoiseGit", 3, IDI_QUESTION, CString(MAKEINTRESOURCE(IDS_DELETEBUTTON)), CString(MAKEINTRESOURCE(IDS_DROPONESTASH)), CString(MAKEINTRESOURCE(IDS_ABORTBUTTON)));
		if (choose == 1)
		{
			CString sCmd;
			sCmd.Format(L"git.exe stash clear");
			CString out;
		...
		{
			CString sCmd;
			sCmd.Format(L"git.exe stash drop refs/stash@{0}");
			CString out;
			if (g_Git.Run(sCmd, &out, CP_UTF8))
				CMessageBox::Show(nullptr, out, L"TortoiseGit", MB_OK | MB_ICONERROR);
			return TRUE;
	...
	CString msg;
	msg.Format(IDS_PROC_DELETEBRANCHTAG, (LPCWSTR)ref);
	...
	switch (cmd&0xFFFF)
		{
			...
			case ID_GNUDIFF1: // compare with WC, unified
			{
				...
				if(!r1->m_CommitHash.IsEmpty())
				{
					...
					CAppUtils::StartShowUnifiedDiff(nullptr, CTGitPath(), L"HEAD", CTGitPath(), GitRev::GetWorkingCopy(), false, false, false, bMerge, bCombine);
			}
			break;
		...
		case ID_LOG_VIEWRANGE_REACHABLEFROMONLYONE:
			{
				...
				cmdline.Format(L"/command:log /path:\"%s\" /range:\"%s%s%s\"",
					(LPCWSTR)g_Git.CombinePath(m_Path), (LPCWSTR)pLastEntry->m_CommitHash.ToString(), (LPCWSTR)sep, (LPCWSTR)pSelLogEntry->m_CommitHash.ToString());
		...
		case ID_COMBINE_COMMIT:
		{
			...
			try
			{
				sCmd.Format(L"git.exe reset --hard %s --", (LPCWSTR)pFirstEntry->m_CommitHash.ToString());
				if(g_Git.Run(sCmd, &out, CP_UTF8))
				{
				...
				sCmd.Format(L"git.exe reset --mixed %s --", (LPCWSTR)hashLast.ToString());
				if(g_Git.Run(sCmd, &out, CP_UTF8))
				{
					MessageBox(out, L"TortoiseGit", MB_OK | MB_ICONERROR);
				...
				for (int i = 0; i < PathList.GetCount(); ++i)
				{
					...
					{
						sCmd.Format(L"git.exe add -- \"%s\"", (LPCWSTR)PathList[i].GetGitPathString());
						if (g_Git.Run(sCmd, &out, CP_UTF8))
						{
							MessageBox(out, L"TortoiseGit", MB_OK | MB_ICONERROR);
					...
					{
						sCmd.Format(L"git.exe rm -- \"%s\"", (LPCWSTR)PathList[i].GetGitPathString());
						if (g_Git.Run(sCmd, &out, CP_UTF8))
						{
							MessageBox(out, L"TortoiseGit", MB_OK | MB_ICONERROR);
			...
			{
				CMessageBox::Show(nullptr, CUnicodeUtils::GetUnicode(CStringA(e.what())), L"TortoiseGit", MB_OK | MB_ICONERROR);
				sCmd.Format(L"git.exe reset --hard %s --", (LPCWSTR)headhash.ToString());
				out.Empty();
				if(g_Git.Run(sCmd, &out, CP_UTF8))
		...
		case ID_REFLOG_DEL:
			{
				...
				if (GetSelectedCount() > 1)
					...
					str.Format(IDS_PROC_DELETEREF, (LPCWSTR)pSelLogEntry->m_Ref);

				if (CMessageBox::Show(nullptr, str, L"TortoiseGit", 1, IDI_QUESTION, CString(MAKEINTRESOURCE(IDS_DELETEBUTTON)), CString(MAKEINTRESOURCE(IDS_ABORTBUTTON))) == 2)
					return;
				...
				for (auto revIt = refsToDelete.crbegin(); revIt != refsToDelete.crend(); ++revIt)
				{
					...
					if (ref.Find(L"stash") == 0)
						sCmd.Format(L"git.exe stash drop %s", (LPCWSTR)ref);
					else
						sCmd.Format(L"git.exe reflog delete %s", (LPCWSTR)ref);
		...
		case ID_REPOBROWSE:
			{
				CString sCmd;
				sCmd.Format(L"/command:repobrowser /path:\"%s\" /rev:%s", (LPCWSTR)g_Git.m_CurrentDir, (LPCWSTR)pSelLogEntry->m_CommitHash.ToString());
				CAppUtils::RunTortoiseGitProc(sCmd);
			}
			break;
		case ID_PUSH:
		...
		case ID_CLEANUP:
			{
				CString sCmd;
				sCmd.Format(L"/command:cleanup /path:\"%s\"", (LPCWSTR)g_Git.m_CurrentDir);
				CAppUtils::RunTortoiseGitProc(sCmd);
			}
			break;
		...
		case ID_SHOWBRANCHES:
			{
				CString sCmd;
				sCmd.Format(L"git.exe branch -a --contains %s", (LPCWSTR)pSelLogEntry->m_CommitHash.ToString());
		...
		case ID_OPEN:
			{
				...
				CString sInfoLine;
				sInfoLine.Format(IDS_PROGRESSGETFILEREVISION, m_path.GetWinPath(), (LPCWSTR)revSelected.ToString());
				...
				if (!Cat(m_path, GitRev(GitRev::REV_HEAD), revSelected, tempfile))
				{
					...
					if (!Cat(m_path, revSelected, revSelected, tempfile))
					{
						...
						CMessageBox::Show(this->m_hWnd, GetLastErrorMessage(), L"TortoiseGit", MB_ICONERROR);
						EnableOKButton();
						break;
					}
					bSuccess = true;
		...
		case ID_BLAME:
			{
				...
				if (dlg.DoModal() == IDOK)
				{
					...
					{
						CMessageBox::Show(this->m_hWnd, blame.GetLastErrorMessage(), L"TortoiseGit", MB_ICONERROR);
					}
				}
			}
			break;
		case ID_EXPORT:
			{
				...
				sCmd.Format(L"%s /command:export /path:\"%s\" /revision:%ld",
					...
					(LPCWSTR)pathURL, (LONG)revSelected);
