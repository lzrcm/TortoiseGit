...
, m_eventStop(eventStop)
{
	...
	CString userAgent;
	userAgent.Format(L"TortoiseGit %s; %s; Windows%s%s %s%s%s", _T(STRFILEVER), _T(TGIT_PLATFORM), m_sWindowsPlatform.IsEmpty() ? L"" : L" ", (LPCWSTR)m_sWindowsPlatform, (LPCWSTR)m_sWindowsVersion, m_sWindowsServicePack.IsEmpty() ? L"" : L" ", (LPCWSTR)m_sWindowsServicePack);
