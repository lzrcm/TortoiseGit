	...
	, m_bNavigatingWithSelect(false)
{
	...
	str.Replace(L":",L"_");
	str=CString(L"Software\\TortoiseGit\\LogDialog\\AllBranch\\")+str;
...
void CLogDlg::TogglePatchView()
{
	...
	if (!IsWindow(m_patchViewdlg.m_hWnd))
	{
		...
		viewPatchEnabled = g_Git.GetConfigValueBool(L"tgit.logshowpatch");
...
void CLogDlg::OnBnClickedJumpUp()
{
	...
	for (int i = index - 1; i >= 0; i--)
	{
		...
		else if (jumpType == JumpType_Tag || jumpType == JumpType_TagFF)
		{
			...
			{
				if (refList[j].Left(10) == L"refs/tags/")
				{
					found = true;
					break;
				}
		...
		else if (jumpType == JumpType_Branch || jumpType == JumpType_BranchFF)
		{
			...
			for (size_t j = 0; j < refList.size(); ++j)
			{
				if (refList[j].Left(11) == L"refs/heads/" || refList[j].Left(13) == L"refs/remotes/")
				{
					found = true;
					break;
				}
