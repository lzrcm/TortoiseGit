		...
		case ID_LOGSUBMODULE:
			{
				...
				while (pos)
				{
					...
					sCmd += L" /endrev:" + m_rev1.m_CommitHash.ToString();
		...
		case ID_SAVEAS:
			{
				if (m_cFileList.GetSelectedCount() > 0)
				{
					...
					CTGitPath savePath;
					CString pathSave;
					if (!CAppUtils::FileOpenSave(pathSave, nullptr, IDS_REPOBROWSE_SAVEAS, IDS_TEXTFILEFILTER, false, m_hWnd, L".txt"))
					{
						break;
					}
...
{
	CLogDlg dlg;
	dlg.SetRange(m_rev2.m_CommitHash.ToString() + L".." + m_rev1.m_CommitHash.ToString());
	dlg.DoModal();
}
