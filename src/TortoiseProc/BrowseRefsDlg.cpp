...
{
	if (ref.Left(13) == L"refs/remotes/")
		ref = ref.Mid(13);
	else if (ref.Left(8) == L"remotes/")
		ref = ref.Mid(8);
	...
	{
		if (ref.Left(list[i].GetLength() + 1) == list[i] + L"/")
		{
			remote = list[i];
...
BOOL CBrowseRefsDlg::OnInitDialog()
{
	...
	m_ColumnManager.ReadSettings(dwDefaultColumns, 0, L"BrowseRefs", _countof(columnNames), columnWidths);
	m_bPickedRefSet = false;
...
bool CBrowseRefsDlg::ConfirmDeleteRef(VectorPShadowTree& leafs)
{
	...
	if(bIsBranch)
	{
		if(leafs.size() == 1)
		{
			...
			csMessage.Format(IDS_PROC_DELETEBRANCHTAG, (LPCWSTR)branchToDelete);
	...
	else if(leafs[0]->IsFrom(L"refs/tags/"))
	{
		if(leafs.size() == 1)
		{
			...
			csMessage.Format(IDS_PROC_DELETEBRANCHTAG, (LPCWSTR)tagToDelete);
		}
		else
			csMessage.Format(IDS_PROC_DELETENREFS, leafs.size());
	}

	return CMessageBox::Show(m_hWnd, csMessage, L"TortoiseGit", MB_YESNO | mbIcon) == IDYES;
}
...
bool CBrowseRefsDlg::DoDeleteRef(CString completeRefName)
{
	...
	if (bIsRemoteBranch)
	{
		...
		{
			CMessageBox::Show(m_hWnd, g_Git.GetGitLastErr(L"Could not delete remote ref.", CGit::GIT_CMD_PUSH), L"TortoiseGit", MB_OK | MB_ICONERROR);
			sysProgressDlg.Stop();
			BringWindowToTop();
			return false;
		}
	...
	else if (bIsBranch)
	{
		...
		{
			CMessageBox::Show(m_hWnd, g_Git.GetGitLastErr(L"Could not delete reference.", CGit::GIT_CMD_DELETETAGBRANCH), L"TortoiseGit", MB_OK | MB_ICONERROR);
			return false;
		}
	}
...
void CBrowseRefsDlg::ShowContextMenu(CPoint point, HTREEITEM hTreePos, VectorPShadowTree& selectedLeafs)
{
	...
	if(selectedLeafs.size()==1)
	{
		...
		else if(selectedLeafs[0]->IsFrom(L"refs/remotes/"))
		{
			...
			if (SplitRemoteBranchName(selectedLeafs[0]->GetRefName(), remoteName, remoteBranch))
				...
				fetchFromCmd.Format(IDS_PROC_BROWSEREFS_FETCHFROM, (LPCWSTR)remoteName);
		}
	...
	else if(selectedLeafs.size() == 2)
	{
		...
		CString menu;
		menu.Format(IDS_SHOWLOG_OF, (LPCWSTR)GetTwoSelectedRefs(selectedLeafs, m_sLastSelected, L".."));
	...
	if (hTreePos && selectedLeafs.empty())
	{
		...
		if(pTree->IsFrom(L"refs/remotes"))
		{
			...
			if(selectedLeafs.empty())
			{
				...
				{
					CString temp;
					temp.Format(IDS_PROC_BROWSEREFS_FETCHFROM, (LPCWSTR)remoteName);
		...
		if(pTree->IsFrom(L"refs/tags"))
		{
			...
			if (!remotes.empty())
			{
				...
				{
					temp.Format(IDS_DELETEREMOTETAGON, (LPCWSTR)*it);
	...
	case eCmd_ViewLog:
		{
			CString sCmd;
			sCmd.Format(L"/command:log /path:\"%s\" /range:\"%s\"", (LPCWSTR)g_Git.m_CurrentDir, (LPCWSTR)g_Git.FixBranchName(selectedLeafs[0]->GetRefName()));
	...
	case eCmd_ViewLogRange:
		{
			CString sCmd;
			sCmd.Format(L"/command:log /path:\"%s\" /range:\"%s\"", (LPCWSTR)g_Git.m_CurrentDir, (LPCWSTR)GetTwoSelectedRefs(selectedLeafs, m_sLastSelected, L".."));
			CAppUtils::RunTortoiseGitProc(sCmd);
		}
		break;
...
//			W_Remotes.DoModal();
	...
	case eCmd_UpstreamDrop:
		{
			CString key;
			key.Format(L"branch.%s.remote", (LPCWSTR)selectedLeafs[0]->GetRefsHeadsName());
			g_Git.UnsetConfigValue(key);
			key.Format(L"branch.%s.merge", (LPCWSTR)selectedLeafs[0]->GetRefsHeadsName());
			g_Git.UnsetConfigValue(key);
		}
		Refresh();
		break;
