...
TEST(CGit, RunGit_BashPipe)
{
	...
	tmpfile.Replace(L"\\", L"/");
	ASSERT_TRUE(CStringUtils::WriteStringToTextFile((LPCWSTR)tmpfile, L"testing piping..."));
	SCOPE_EXIT{ ::DeleteFile(tmpfile); };
	CString pipefile = GetTempFile();
	pipefile.Replace(L"\\", L"/");
	CString pipecmd;
	pipecmd.Format(L"cat < %s", (LPCWSTR)tmpfile);
	ASSERT_TRUE(CStringUtils::WriteStringToTextFile((LPCWSTR)pipefile, (LPCWSTR)pipecmd));
...
TEST(CGit, GetFileModifyTime)
{
	...
	EXPECT_TRUE(CStringUtils::WriteStringToTextFile((LPCWSTR)testFile, L"this is testing fileöäü."));
	...
	Sleep(1200);
	EXPECT_TRUE(CStringUtils::WriteStringToTextFile((LPCWSTR)testFile, L"this is testing fileöü."));
	__int64 time3 = -1;
	isDir = false;
	size = -1;
...
TEST(CGit, LoadTextFile)
{
	...
	msg.Empty();
	EXPECT_TRUE(CStringUtils::WriteStringToTextFile((LPCWSTR)testFile, L"this is testing\nfileöäü."));
	EXPECT_TRUE(CGit::LoadTextFile(testFile, msg));
	EXPECT_STREQ(L"this is testing\nfileöäü.\n", msg);

	msg.Empty();
	EXPECT_TRUE(CStringUtils::WriteStringToTextFile((LPCWSTR)testFile, L"this is\r\ntesting\nfileöäü.\r\n\r\n"));
	EXPECT_TRUE(CGit::LoadTextFile(testFile, msg));
...
TEST_P(CBasicGitWithEmptyRepositoryFixture, IsInitRepos_GetInitAddList)
{
	...
	EXPECT_TRUE(CStringUtils::WriteStringToTextFile((LPCWSTR)testFile, L"this is testing file."));
	EXPECT_EQ(0, m_Git.GetInitAddList(addedFiles));
	EXPECT_TRUE(addedFiles.IsEmpty());
...
static void FETCHHEAD(CGit& m_Git, bool isBare)
{
	...
	EXPECT_TRUE(CStringUtils::WriteStringToTextFile((LPCWSTR)testFile, L"b9ef30183497cdad5c30b88d32dc1bed7951dfeb		branch 'master' of https://code.google.com/p/tortoisegit\n737878a4e2eabfa4fab580867c2b060c70999d31	not-for-merge	branch 'extend_hooks' of https://code.google.com/p/tortoisegit\n"));
	...
	EXPECT_TRUE(CStringUtils::WriteStringToTextFile((LPCWSTR)testFile, L"737878a4e2eabfa4fab580867c2b060c70999d31	not-for-merge	branch 'extend_hooks' of https://code.google.com/p/tortoisegit\nb9ef30183497cdad5c30b88d32dc1bed7951dfeb		branch 'master' of https://code.google.com/p/tortoisegit\n"));
...
TEST_P(CBasicGitWithEmptyRepositoryFixture, CheckCleanWorkTree)
{
	...
	EXPECT_TRUE(CStringUtils::WriteStringToTextFile((LPCWSTR)testFile, L"Overwriting this testing file."));
	...
	EXPECT_TRUE(CStringUtils::WriteStringToTextFile((LPCWSTR)testFile, L"this is ANOTHER testing file."));
	EXPECT_TRUE(m_Git.CheckCleanWorkTree());
	EXPECT_TRUE(m_Git.CheckCleanWorkTree(true));
...
TEST_P(CBasicGitWithTestRepoFixture, GetOneFile)
{
	...
	EXPECT_TRUE(CStringUtils::WriteStringToTextFile((LPCWSTR)attributesFile, L"*.enc filter=openssl\n"));

	CString encryptedFileOne = m_Git.m_CurrentDir + L"\\1.enc";
	EXPECT_TRUE(CStringUtils::WriteStringToTextFile((LPCWSTR)encryptedFileOne, L"This should be encrypted...\nAnd decrypted on the fly\n"));

	CString encryptedFileTwo = m_Git.m_CurrentDir + L"\\2.enc";
	EXPECT_TRUE(CStringUtils::WriteStringToTextFile((LPCWSTR)encryptedFileTwo, L"This should also be encrypted...\nAnd also decrypted on the fly\n"));
...
TEST_P(CBasicGitWithTestRepoFixture, Config)
{
	...
	EXPECT_TRUE(CStringUtils::WriteStringToTextFile((LPCWSTR)gitConfig, L"[booltest]\nistrue"));
...
TEST_P(CBasicGitWithTestRepoFixture, GetWorkingTreeChanges)
{
	...
	EXPECT_TRUE(CStringUtils::WriteStringToTextFile((LPCWSTR)testFile, L"*.enc filter=openssl\n"));
	EXPECT_EQ(0, m_Git.GetWorkingTreeChanges(list, false, nullptr));
	EXPECT_TRUE(list.IsEmpty());
	list.Clear();
	EXPECT_EQ(0, m_Git.GetWorkingTreeChanges(list, true, nullptr));
	...
	EXPECT_EQ(0, m_Git.Run(L"git.exe add copy/utf8-nobom.txt", &output, CP_UTF8));
	EXPECT_TRUE(output.IsEmpty());
	EXPECT_TRUE(CStringUtils::WriteStringToTextFile((LPCWSTR)testFile, L"now with different content after staging"));
	EXPECT_EQ(0, m_Git.GetWorkingTreeChanges(list, false, nullptr));
	ASSERT_EQ(1, list.GetCount());
	EXPECT_EQ(CTGitPath::LOGACTIONS_MODIFIED, list.GetAction());
	EXPECT_STREQ(L"copy/utf8-nobom.txt", list[0].GetGitPathString());
